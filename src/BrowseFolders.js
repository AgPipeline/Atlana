...
class BrowseFolders extends Component {
  ...
  folder_instance = null;       // The definition of remove folder
  interface = null;             // The active interface to remote storage
  fetch_pending = null;         // The current working fetch request
  fetch_pending_id = 0;         // Value used to keep track of latest fetch
  ...
  connectRequestFinish(request_id, results, finish_cb) {
    console.log("connectRequestFinish", request_id,results,finish_cb);
  ...
  connectRequestStart(connect_info, connect_cb) {
    ...
    console.log("connect start",connect_info, connect_cb, this.interface);
